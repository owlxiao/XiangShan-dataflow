...
class SimFPGA(implicit p: Parameters) extends Module {
  ...
  val l_simMMIO = LazyModule(new SimFPGAMMIO(soc.peripheral_adapter.l_widthAdapter.slave.in.head._2))
  val simMMIO = Module(l_simMMIO.module)
  l_simMMIO.io_axi4 <> soc.peripheral

  if(!useDRAMSim){
    val l_simAXIMem = LazyModule(new AXI4RAMWrapper(
      soc.lazy_module_top.misc.memAXI4SlaveNode, 8L * 1024 * 1024 * 1024, useBlackBox = true
    ))
